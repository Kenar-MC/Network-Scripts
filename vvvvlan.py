...
def get_mac_counts_per_vlan(output):
    ...
    print(f"Total lines in output: {len(lines)}")  # Debug print
    for line in lines:
...
try:
    ...
    output = ''
    while True:
        time.sleep(1)  # Bir saniye bekleyerek daha stabil bir çıktı elde edebiliriz
        if ssh_session.recv_ready():
            chunk = ssh_session.recv(65535).decode()
            output += chunk
            print(f"Received chunk: {chunk[:200]}...")  # Chunk'ın ilk 200 karakterini yazdır
        else:
            break
